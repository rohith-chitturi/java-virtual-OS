...
public class WriteCommand implements Command {
    ...
    public WriteCommand(FileSystemService fsService) {
    ...
    public CommandResult execute(String[] args, ShellContext context) {
        if (args.length < 2) {
            return CommandResult.failure("write: missing operand (Usage: write <file> <content>)");
        }
        String path = args[0];
        String content = String.join(" ", Arrays.copyOfRange(args, 1, args.length));
        return fsService.writeFile(path, content, context);
    }
    ...
    public String getDescription() {
        return "Write text to a file, replacing its contents";
    }
}
